...
export async function generateSummary(
  ...
  const featureArticles = articles.filter(a => 
    (a.patchData?.type || a.gitCommitData?.type) === 'feature'
  )
  const bugfixArticles = articles.filter(a => 
    (a.patchData?.type || a.gitCommitData?.type) === 'bugfix'
  )
  const otherArticles = articles.filter(a => 
    (a.patchData?.type || a.gitCommitData?.type) === 'other' || 
  ...
  if (bugfixArticles.length > 0) {
    ...
    if (bugfixSummary) {
      if (featureArticles.length > 0) parts.push('')
      parts.push(`## Bugfix`)
  ...
  if (otherArticles.length > 0) {
    if (featureArticles.length > 0 || bugfixArticles.length > 0) parts.push('')
    parts.push(`## Other`)
